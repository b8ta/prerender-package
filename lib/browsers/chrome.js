...
chrome.setUpEvents = function(tab) {
	return new Promise((resolve, reject) => {
		...
		Promise.all([
			...
			Log.enable(),
			Console.enable()
		]).then(() => Network.setRequestInterception({
			patterns: [{
				urlPattern: '*',
			...
			Network.requestIntercepted(({ interceptionId, request }) => {
				...
				if (tab.prerender.logRequests || this.options.logRequests) util.log('intercept', request.url);
				if (request.url.startsWith('blob:')) {
					// abort the request
					if (tab.prerender.logRequests || this.options.logRequests) util.log('aborted', request.url);
					continueParams.errorReason = 'Aborted';
				} else {
					if (tab.prerender.logRequests || this.options.logRequests) util.log('continued', request.url);
